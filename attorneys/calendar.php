...
<div class="container py-5">
    ...
    <div class="container py-2">
        <h2 class="text-center page-heading">
            <i class="far fa-calendar-alt text-primary mr-2"></i><?= date('F Y') ?> Zoom Meetings Calendar
        </h2>
    </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    $meetingsByDay = [];
                    $sql = "SELECT * FROM `meetings` WHERE status='Pending'";
                    $res = $conn->query($sql);
                    while ($row = $res->fetch_assoc()) {
                        ...
                        $caseid  = $row['caseid'];
$row1    = $conn->query("SELECT * FROM `case`
                         WHERE id = '$caseid' AND attid = '$attid'")
               ->fetch_assoc();

if (!$row1) {
    // Case not found for this attorney –- skip this meeting
    continue;
}
...
$row2   = $conn->query("SELECT * FROM `user`
                        WHERE id = '$userID'")
              ->fetch_assoc();

$name = $row2 ? $row2['fname'].' '.$row2['lname'] : 'Unknown User';
...
function renderCalendarRow(int $day,
                           array $meetingsByDay,
                           int $year,
                           int $month,
                           int $todayDay): void
{
    ...
    if (!empty($meetingsByDay[$day])) {
        foreach ($meetingsByDay[$day] as $idx => $m) {
            echo "<span class='meeting-badge'>{$m['name']}"
               . "<span class='meeting-time'>{$m['time']} - {$m['endtime']}</span></span><br>";
        }
    } else {
        echo "<span class='no-meetings'>No meetings scheduled</span>";
    }
    echo "</td>";

    /* ---------- Description column with modal(s) ---------- */
    echo "<td>";
    if (!empty($meetingsByDay[$day])) {
        foreach ($meetingsByDay[$day] as $idx => $m) {
            $modalId = "modal_{$day}_{$idx}";
            echo "<button type='button' class='btn btn-sm btn-primary mb-2'
                         data-toggle='modal' data-target='#$modalId'>View</button>";
              ...
              <div class='modal-dialog' role='document'>
                <div class='modal-content'>
                  <div class='modal-header'>
                    <h5 class='modal-title' id='{$modalId}Label'>Meeting Description</h5>
...
for ($d = $todayDay; $d <= $daysInMonth; $d++) {
    renderCalendarRow($d, $meetingsByDay, $currentYear, $currentMonth, $todayDay);
}
