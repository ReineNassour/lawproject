...
<div class="container py-5">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    while ($row = $res->fetch_assoc()) {
                        ...
                        $caseid  = $row['caseid'];
                        $row1    = $conn->query("SELECT * FROM `case` WHERE id='$caseid'")->fetch_assoc();
                        $userID  = $row1['userid'];
                        $attid=$row1['attid'];
                         $row4    = $conn->query("SELECT * FROM `user` WHERE id='$attid'")->fetch_assoc();
                          $attname    = $row4['fname']." ".$row4['lname'];
    ...
    'attorney' => $attname
                 ...
                 /* -----------------------------------------------------------
 *  Build the table in two passes so the list “wraps”:
 *  1.  Today → end-of-month
 *  2.  1     → yesterday
 * ----------------------------------------------------------- */

function printDayRow(int $day, array $meetingsByDay,
                     int $year, int $month, int $todayDay): void
{
    ...
    if (!empty($meetingsByDay[$day])) {
        foreach ($meetingsByDay[$day] as $m) {
            echo "<span class='meeting-badge'>{$m['name']}"
               . "<span class='meeting-time'>{$m['time']}</span></span>";
        }
    } else {
...
/* ---------- 1) Today ­→ end of the month ---------- */
for ($d = $todayDay; $d <= $daysInMonth; $d++) {
    printDayRow($d, $meetingsByDay, $currentYear, $currentMonth, $todayDay);
}

/* ---------- 2) Beginning of the month ­→ yesterday ---------- */
