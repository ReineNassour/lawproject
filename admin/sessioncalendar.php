...
while ($row = $res->fetch_assoc()) {
    ...
    // Get case info
    $case = $conn->query("SELECT * FROM `case` WHERE id='$caseid'")->fetch_assoc();
    if (!$case) continue;
    ...
    // Get client name
    $client = $conn->query("SELECT * FROM `user` WHERE id='$userID'")->fetch_assoc();
    $clientName = ($client) ? $client['fname'] . " " . $client['lname'] : 'Unknown';

    // Get attorney name
    $attorney = $conn->query("SELECT * FROM `user` WHERE id='$attid'")->fetch_assoc();
    $attorneyName = ($attorney) ? $attorney['fname'] . " " . $attorney['lname'] : 'Unknown';
...
function renderDayRow(int $day,
                      array $meetingsByDay,
                      int $year,
                      int $month,
                      int $todayDay,
                      bool $isNextMonthRow = false): void
{
    $date    = sprintf('%04d-%02d-%02d', $year, $month, $day);
    $dayName = date('l', strtotime($date));
    $isToday = ($day === $todayDay && !$isNextMonthRow) ? 'today' : '';

    // tiny label for rows that appear *after* the end-of-month
    $note = $isNextMonthRow ? " <small class='text-muted'>(next&nbsp;month)</small>" : '';

    echo "<tr class='$isToday'>";

    /* ---------- Date + Day ---------- */
    echo "<td class='date-cell'>$day$note</td>";
    echo "<td>$dayName</td>";
    ...
    if (!empty($meetingsByDay[$day])) {
        foreach ($meetingsByDay[$day] as $m) {
            echo "<span class='meeting-badge'>{$m['client']}"
               . "<span class='meeting-time'>{$m['time']}</span></span><br>";
        }
    } else {
    ...
    /* ---------- Description (modal per session) ---------- */
    echo "<td>";
    if (!empty($meetingsByDay[$day])) {
        $modalIndex = 0;
        foreach ($meetingsByDay[$day] as $m) {
            $modalId = "descModal_{$day}_{$modalIndex}";
            echo "
                  ...
                  <div class='modal-dialog modal-dialog-centered' role='document'>
                    <div class='modal-content'>
                      <div class='modal-header'>
                        <h5 class='modal-title' id='{$modalId}Label'>Session Description</h5>
        ...
        echo "<span class='no-meetings'>–</span>";
    }
    echo "</td>";
...
for ($d = $todayDay; $d <= $daysInMonth; $d++) {
    renderDayRow($d, $meetingsByDay, $currentYear, $currentMonth, $todayDay, false);
}


for ($d = 1; $d < $todayDay; $d++) {
    renderDayRow($d, $meetingsByDay, $currentYear, $currentMonth, $todayDay, true);
}
